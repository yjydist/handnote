...
Handnote is a Bun/TypeScript CLI that turns one handwritten-note image into a faithful electronic `note.json` and a locally rendered `note.png`. A Mastra agent may inspect source regions, write and patch the document, review the render against the source, and finalize only after an unchanged revision passes a later-step review. Model summaries, page commentary, and audit sections are not part of the finished note.
...
Each run directory is named with local time and a sanitized source stem. It always retains the byte-identical `original.<ext>`, `run.json`, and `session/events.jsonl`. Only complete or usable partial runs contain root-level `note.json` and `note.png`. `note.json` contains only the electronic-note document (`title` when explicitly source-backed, plus `sections`); uncertainty and conservative-correction audits are session-only. When intermediate image retention is disabled, inspection images and revision HTML/PNG files are removed after their hashes are recorded.

The document and audit contracts are intentionally separate. `write_document` accepts `{document,audit}`. Audit items point to an existing document title, section title, or block; their selected value is rendered as ordinary note content, while candidates, evidence, confidence, and correction details are recorded only in the revision event. Corrections require confidence of at least `0.95`. The strict document schema rejects the former top-level `summary`, `corrections`, and `uncertainties` fields.
