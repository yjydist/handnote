...
The note and audit contracts are intentionally separate. `write_note` and `revise_note` accept `{markdown,audit}`. The markdown must be strict GFM: raw HTML, links (including bare URLs), frontmatter, and non-local images are rejected with a repairable `invalid_markdown` error; mathematics uses `$...$` / `$$...$$`, diagrams use `mermaid` fenced code blocks, and figures use `![caption](assets/figures/figure-NNN.png)` paths returned by `capture_figure`. Audit items locate their target with a quote copied verbatim from the markdown (plus `occurrence` for repeats); their selected value is rendered as ordinary note content, while candidates, evidence, confidence, and correction details are recorded only in the revision event. Corrections require confidence of at least `0.95`. `finalize_note` re-hashes the on-disk revision file and refuses to finalize on a mismatch, binding the final artifacts to the reviewed revision.
