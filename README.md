...
Each run directory is named with local time and a sanitized source stem. It always retains the byte-identical `original.<ext>`, `run.json`, and `session/events.jsonl`. Only complete or usable partial runs contain root-level `note.md` and `note.png`. `note.md` is the finalized revision's GFM markdown verbatim; every committed revision is also retained byte-identical under `revisions/revision-NNN.md`, and `assets/figures/` keeps captured source crops, so each revision is independently rebuildable from disk. When intermediate image retention is disabled, inspection images and revision HTML/PNG files are removed after their hashes are recorded; `revisions/` and `assets/` always survive.

The note and audit contracts are intentionally separate. `write_note` and `revise_note` accept `{markdown,audit}`. The markdown must be strict GFM: raw HTML, links (including footnotes and bare URLs), frontmatter, reference-style images, and non-local images are rejected with a repairable `invalid_markdown` error; mathematics uses `$...$` / `$$...$$` while literal dollar signs must be escaped as `\$`, diagrams use lowercase `mermaid` fenced code blocks, and figures use `![caption](assets/figures/figure-NNN.png)` paths returned by `capture_figure`. Audit items locate their target in rendered-visible Markdown text (plus `occurrence` for repeats), not in raw markup; their selected value is rendered as ordinary note content, while candidates, evidence, confidence, and correction details are recorded only in the revision event. Corrections require confidence of at least `0.95`. `finalize_note` re-hashes the on-disk revision file and refuses to finalize on a mismatch, binding the final artifacts to the reviewed revision.
