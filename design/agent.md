...
`write_note` 与 `revise_note` 共享 `commitNoteDraft` (`src/tools/write-note.ts:52`): `revisionDraftSchema` parse (结构错误或无渲染依赖内容文档的 locator 失败 → 可修复 `invalid_audit`) → `parseNoteMarkdown` 源码与静态语义校验 (失败 → 可修复 `invalid_markdown`, issues 格式化列出) → 渲染并提取瞬时 Mermaid / KaTeX 语义证据 → 拒绝渲染后出现的链接 / 媒体 / 事件属性及无实际内容的文档 (可修复 `invalid_markdown`) → 最终 locator 校验 (失败 → 可修复 `invalid_audit`) → revision markdown 原字节落盘 → `state.commit`. 瞬时证据不进入 state / manifest / session. 渲染是隐式的, 没有 render_note 工具.
