...
- Markdown 契约: GFM Markdown 是 canonical 持久化格式, 使用标准 unified 管线与 HTML 清理，允许链接、脚注及引用式本地图片；AST 不持久化。审计精确定位 Markdown 源码，视觉内容由审查判断；图片仍限定在当前运行的 assets/figures/ (详见 [markdown.md](markdown.md)).
...
| `src/document.ts` | 契约 schema、`Region` 与 Markdown 源码 quote-locator 审计 |
| `src/markdown.ts` | 标准 unified 编译、HTML 清理、本地资源内联、结构统计与锚点 |
...
| `src/renderer.ts` | 消费编译 HTML、截图、布局体检与超高页拼合 |
