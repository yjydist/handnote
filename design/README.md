...
- 输出: 一次完整运行产出 `note.json` (结构化 `NoteDocument`) + `note.png` (自包含 HTML 渲染出的图) + `run.json` (运行 manifest) + `session/events.jsonl` (脱敏 session 日志).
...
| `src/document.ts` | 核心数据模型, `NoteDocument` / `Block` / `RevisionAudit` 等 schema (`src/document.ts:183`) |
| `src/patch.ts` | 增量修订契约, `patchOperationSchema` 与原子 `applyPatch` (`src/patch.ts:128`) |
| `src/image.ts` | 图像处理, `inspect_source` 裁剪, 模型预览生成, 元数据 (`src/image.ts:239`) |
...
| `src/tools/` | 5 个 agent 工具目录, 入口 `index.ts` 组装; `shared.ts` 公共闭包状态 (`src/tools/index.ts:11`) |
| `src/renderer.ts` | HTML 渲染与截图, 布局体检与超高页拼合 (`src/renderer.ts:397`) |
...
| `tests/` | 单元与集成测试, 覆盖 document / config / run / renderer / agent-loop 等 |
...
| [data-model.md](data-model.md) | 核心数据模型 (`src/document.ts`) |
| [patch.md](patch.md) | Patch 契约 (`src/patch.ts`) |
