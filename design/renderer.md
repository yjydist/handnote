...
- 输入为 `NoteMarkdown` IR (含锚点 mdast), 经 `noteMarkdownToHtml` (`src/markdown.ts:365`) 的 unified 管线生成 HTML 主体 (详见 [markdown.md](markdown.md)); `renderDocument` (`src/renderer.ts:288`) 负责套自包含外壳与截图, 并返回持久化 `render` 与仅供提交前 audit 校验的瞬时 `auditText`. Mermaid 证据来自成功 SVG 中可见的 `text` / `foreignObject` 并按视觉 reading order 排序; math 证据来自排除原始 TeX annotation 的 KaTeX MathML 语义内容, KaTeX error 则使用实际显示的 fallback. 这些证据不进入 state / manifest / session.
