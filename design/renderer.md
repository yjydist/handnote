...
- 输入为 `NoteMarkdown` IR (含锚点 mdast), 经 `noteMarkdownToHtml` (`src/markdown.ts:365`) 的 unified 管线生成 HTML 主体 (详见 [markdown.md](markdown.md)); `renderDocument` (`src/renderer.ts:271`) 负责套自包含外壳与截图, 并返回持久化 `render` 与仅供提交前 audit 校验的瞬时 `mermaidTextBlocks`. 后者来自成功 Mermaid SVG 中可见的 `text` / `foreignObject`, 按视觉 reading order 排序, 不进入 state / manifest / session.
