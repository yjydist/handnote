...
import { HandnoteError, safeErrorMetadata } from "./errors.ts";
import { classifyProviderError } from "./provider/classify.ts";
import {
  finiteNumber,
  isRetryableStatus,
  record,
...
async function repairToolArgumentResponse(
  response: Response,
  ...
  const transport = async (
    ...
    for (let attempt = 0; attempt <= config.maxRetries; attempt++) {
      ...
      try {
        ...
        return await repairToolArgumentResponse(
          response,
          recorder,
          step,
          attempt + 1,
...
export function createModel(
  ...
  const transport = createRetryingFetch(config.model, recorder, state, stats);
