...
An append or flush failure disables the active recorder, so later diagnostics may appear only on stderr. The failing append can leave no new bytes, an incomplete tail, or a complete event whose flush failed. A new recorder resumes from the complete events on disk; only writing recovery repairs an incomplete tail. Do not infer transaction success from a complete but unconfirmed event, or suggest continuing to use the failed recorder.
