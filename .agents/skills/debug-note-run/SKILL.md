...
For a complete run, find the `note.finalized` event and use its revision number to select the matching `document.revision.committed` event. Read uncertainties and conservative corrections from `data.audit` on that revision. Before interpreting those targets, verify that the committed event hash matches the revision bytes and the finalized hash chain. A matching successful commit is the evidence that its audit targets passed the then-current rendered-visible-text validation at commit time. Do not require a target quote to be a raw Markdown substring: Markdown formatting, decoded entities, rendered Mermaid labels, and rendered math semantics can all differ from source syntax. Use `note.png` or a retained revision render only to explain the target and its `occurrence`, and confirm that the selected or corrected content is ordinary note content rather than a visible audit appendix. If the hash chain matches but a rendered target appears absent, report a semantic validation mismatch or validator limitation, not filesystem corruption; if the hashes differ, classify integrity first. Audit metadata remains session-only: `note.md` must contain only the visible GFM note and must not contain audit material or observer commentary.
