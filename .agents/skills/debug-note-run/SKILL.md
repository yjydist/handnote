...
Start with `run.json`, then follow its file references into `session/events.jsonl`, `note.json`, and retained intermediate artifacts. Verify hashes when corruption or mismatched media is plausible. Treat absent intermediate images as expected when `saveIntermediateImages=false` and use their recorded hash plus `retained:false` event metadata.

For a complete run, find the `note.finalized` event and use its revision number to select the matching `document.revision.committed` event. Read uncertainties and conservative corrections from `data.audit` on that revision. Audit metadata is intentionally session-only: `note.json` must contain only the visible document and must not contain top-level `summary`, `corrections`, `uncertainties`, or `audit` fields. Confirm that each audit target exists in the final document and that the selected or corrected content is present as ordinary note content rather than as a visible audit appendix.
